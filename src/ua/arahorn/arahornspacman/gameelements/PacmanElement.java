...
public class PacmanElement extends BaseElement{
	...
	public void draw(GL10 gl, int numberInAtlas) {
		...
		if(time >= 0 && time <= 125)
			...
			n = 7;
		super.draw(gl, pacmanSurfaceCoordinate[1], pacmanSurfaceCoordinate[0] - sizeInSurface, numberInAtlas + n, getRotationByEnum());
	}
	...
	public void updatePacman(float controlX, float controlY) {
		...
		for (int i = 0; i < GameActivity.mapHeight; i++) {
			for (int j = 0; j < GameActivity.mapWidth; j++) {
				if (GameActivity.maskOfMapFile[i][j] == 2 && i == pacmanPosition[0]
						&& j == pacmanPosition[1]) {
...
//	public void setPacmanSurfaceCoordinate (int [] pacmanSurfaceCoordinate) {
//		this.pacmanSurfaceCoordinate = pacmanSurfaceCoordinate;
//	}
